...
const operationsToPairs = expression => {
  ...
  return typeof expression === 'object' ? _.toPairs(expression) || [] : [['=', expression]];
};
...
module.exports.Operations = function (options = {}) {
  const defaultOperators = {
    ...
    $in: (property, operand, builder) => builder.where(property, 'in', typeof operand === 'string' ? operand.split(',') : operand),
    ...
    $or: (property, operand, builder) => builder.where(subQueryBuilder => {
      for (let andExpression of operand) applyOperations(property, andExpression, subQueryBuilder, true);
    })
  };
  ...
  const applyOperations = (propertyName, operations, builder, or = false) => {
    if (or) {
      return builder.orWhere(eagerQueryBuilder => applyOperations(propertyName, operations, eagerQueryBuilder));
    }
