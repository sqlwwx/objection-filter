...
const parseFilter = ({
  limit = 100, fields, offset = 0, order = 'id', where = {}, include = {}
}) => {
  const filter = {
...
const buildCustomFilter = (Model, filter, { trx, ...context } = {}) => {
  ...
  const query = builder.build(parseFilter(filter));
  if (filter.count) {
...
describe('basic filters', function () {
  _.each(testUtils.testDatabaseConfigs, function (knexConfig) {
    describe(knexConfig.client, function() {
      let session, knex, Person, Animal, Movie, MovieVersion;
      ...
      before(function () {
        ...
        Person.find = function (filter) {
          return buildCustomFilter(Person, filter, {
            operators: {
              $equalsLower: (property, operand, builder) => builder.whereRaw('LOWER(??) = LOWER(?)', [property, operand])
            }
          });
        };
      });
